// Note: Sky is optional; if needed, import via CDN or add to import map
import { Flashlight } from './user/Flashlight.js';
...
const Ocean = {
    ...
    async createObstacles() {
        ...
        this.obstacles.push(new Shipwreck(
            this.scene, 40, 0, -60, 0.2, 1.2, { surface: true }
        ));
        ...
        const modelPath = 'js/components/sunken_plane_in_the_morrison_quarry_gltf/scene.gltf';
        // Place multiple plane wrecks on clear floor areas
        ...
        for (const pos of planePositions) {
            const planeObstacle = new ModelObstacle(this.scene, modelPath, {
                position: pos,
                rotationY: planeRotY + Math.random() * 0.2 - 0.1,
                scale: planeScale,
                preserveMaterials: true
            });
            await planeObstacle.whenReady();
            this.obstacles.push(planeObstacle);
        }
    ...
    createBubbles() {
        ...
        // Many more bubbles for better atmosphere
        for (let i = 0; i < 600; i++) {
    ...
    update(deltaTime) {
        ...
        this.camera.position.y = THREE.MathUtils.clamp(this.camera.position.y, -this.oceanDepth + 5, 150);
        ...
        if (depthEl) {
            depthEl.textContent = Math.max(0, Math.round(-this.camera.position.y));
        }
